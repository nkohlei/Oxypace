...
const INITIAL_POSTS = [
    {
        ...
        isPublished: true,
        content: `
          <p class="lead text-lg mb-6 opacity-90">Fiziğin en temel ve sarsıcı sorularından biri, mikroskobik düzeydeki hareket yasalarının tamamı zaman-simetrik iken (yani denklemlerde zamanı ileri veya geri almak hiçbir şeyi değiştirmezken), makroskobik evrende zamanın neden kararlı bir şekilde tek bir yöne aktığıdır. Bu sorunun cevabı, istatistiksel mekanik ve Termodinamiğin İkinci Yasası'nda saklıdır.</p>
          
          <h2 class="text-2xl font-bold mt-8 mb-4">Entropi: Olasılıkların Geometrisi</h2>
          <p class="mb-4 opacity-90">Ludwig Boltzmann'ın formüle ettiği üzere entropi, bir sistemin makroskopik durumuna karşılık gelen mikroskobik konfigürasyonların (mikro-durumların) sayısının bir ölçüsüdür. Basit bir ifadeyle, düzenli bir durumun (düşük entropi) gerçekleşme olasılığı çok düşükken, dağınık ve düzensiz durumların (yüksek entropi) gerçekleşme olasılığı ezici derecede yüksektir. İkinci yasa, kapalı bir sistemin entropisinin zamanla her zaman artma eğiliminde olduğunu söyler.</p>
          
          <blockquote class="border-l-4 border-accent pl-4 my-6 italic opacity-80">
            "Zaman oku, evrenin düzenden düzensizliğe doğru yaptığı kaçınılmaz istatistiksel yolculuğun makroskopik bir yansımasıdır."
          </blockquote>

          <h2 class="text-2xl font-bold mt-8 mb-4">Geçmiş Hipotezi ve Kozmolojik Başlangıç</h2>
          <p class="mb-4 opacity-90">Eğer evren zamanla düzensizliğe gidiyorsa, bu durum geçmişte çok daha düzenli (düşük entropili) olduğu anlamına gelir. Büyük Patlama anında evrenin neden bu kadar olağanüstü derecede düşük entropiye sahip olduğu kozmolojinin en büyük bilmecelerinden biridir. Kozmolog Sean Carroll ve meslektaşları, zamanın yönünü doğrudan bu "Geçmiş Hipotezi"ne bağlar. Eğer Big Bang sırasında madde ve yerçekimi homojen ve ultra-düşük entropili bir yapıda olmasaydı, bugün ne galaksiler, ne yıldızlar ne de zamanı deneyimleyebilecek canlı organizmalar var olabilirdi.</p>
          
          <p class="mb-4 opacity-90">Nihayetinde, kahvenize damlattığınız sütün dağılması, kırılan bir bardağın kendiliğinden birleşmemesi ve biyolojik yaşlanma süreçlerimiz, evrenin kozmik ölçekteki entropi artışıyla doğrudan ilintilidir. Zamanın akışı, yerel bir illüzyon değil, kozmosun en derin termodinamik gerçeğidir.</p>
        `
    },
    {
    ...
    },
    {
        ...
        isPublished: true,
        content: `<p class="lead text-lg mb-6 opacity-90">Galaksilerin dış bölgelerindeki yıldızların dönme hızları, Newton yerçekimi yasaları çerçevesinde beklenenden çok daha yüksektir. Bu anomali, "düz" dönme eğrilerinin varlığına işaret eder ve görünür baryonik maddenin ötesinde büyük miktarda kütlenin varlığını zorunlu kılmaktadır.</p>
          <h2 class="text-2xl font-bold mt-8 mb-4">Karanlık Madde Kanıtları</h2>
          <p class="mb-4 opacity-90">Vera Rubin ve Kent Ford'un 1970'lerdeki gözlemleri, Andromeda galaksisinin dış kollarındaki yıldızların galaktik merkeze yakın yıldızlarla neredeyse aynı hızda döndüğünü ortaya koymuştur. Bu, görünür madde dağılımının öngördüğü Kepler dönme profilinden tamamen farklıdır.</p>`
    },
    {
        ...
        isPublished: true,
        content: `<p class="lead text-lg mb-6 opacity-90">K2'nin 8611 metrelik zirvesinde kısmi oksijen basıncı 33 kPa'ya düşer — deniz seviyesindeki değerin yaklaşık üçte biri. Bu koşulda alveolar PO₂ 35 mmHg civarına geriler ve hemoglobin doygunluğu kritik eşiğin altına iner.</p>
          <h2 class="text-2xl font-bold mt-8 mb-4">Mitokondriyal Solunum Zinciri Çöküşü</h2>
          <p class="mb-4 opacity-90">Bu seviyede sitokrom c oksidaz enziminin oksijen affinitesi sınıra ulaşır. Aerobik metabolizma hızla yerini anaerobik yolağa bırakır ve laktik asidoz başlar. Serebral otoregülasyon mekanizmaları hipoksiyi telafi etmeye çalışırken beyin ödemi riski üstel olarak artar.</p>`
    },
    {
...
function ensureHtmlFormatted(text = '') {
    ...
    for (let line of lines) {
        ...
        if (trimmed.startsWith('## ') || trimmed.startsWith('📌 ')) {
            ...
            htmlResult.push(`<h2 class="text-2xl font-bold mt-8 mb-4">${headerText}</h2>`);
        }
        else if (trimmed.startsWith('### ') || trimmed.startsWith('🔹 ')) {
            if (inList) { htmlResult.push('</ul>'); inList = false; }
            const subText = trimmed.replace(/^(### |🔹 )/, '');
            htmlResult.push(`<h3 class="text-xl font-bold mt-6 mb-3">${subText}</h3>`);
        }
        else if (trimmed.startsWith('> ') || trimmed.startsWith('💬 ')) {
            if (inList) { htmlResult.push('</ul>'); inList = false; }
            const quoteText = trimmed.replace(/^(> |💬 )/, '');
            htmlResult.push(`<blockquote class="border-l-4 border-accent pl-4 my-6 italic opacity-80">"${quoteText}"</blockquote>`);
        }
        else if (trimmed.startsWith('💡 ')) {
            if (inList) { htmlResult.push('</ul>'); inList = false; }
            const infoText = trimmed.replace(/^💡 /, '');
            htmlResult.push(`<div class="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-500 dark:text-blue-300 mb-6">💡 <strong>Not:</strong> ${infoText}</div>`);
        }
        else if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
            if (!inList) {
                htmlResult.push('<ul class="list-disc list-inside space-y-2 mb-6 opacity-90">');
                inList = true;
            }
            ...
            htmlResult.push(`<p class="mb-4 opacity-90">${formattedLine}</p>`);
        }
    }
...
const DEFAULT_CALCULATION_TOOLS = [
    ...
    },
    {
        ...
        image: "https://images.unsplash.com/photo-1506703719100-a0f3a48c0f86?auto=format&fit=crop&w=800&q=80",
        title: {
            tr: "Özel Görelilik ve Lorentz Faktörü Simülatörü",
            en: "Special Relativity & Lorentz Factor Simulator",
        },
        excerpt: {
            tr: "Işık hızına yaklaştıkça uzay-zamanın bükülmesini, Lorentz faktörünü (γ), zaman genleşmesini, uzunluk daralmasını ve göreli kütle artışını canlı simüle edin.",
